...
凍結された `docs/requirements/02_system-requirements.md` を出発点に、**システム全体のアーキテクチャ候補を洗い出し・比較・評価**し、ADR に流し込むための「判断材料」を構造化するフェーズ。

このフェーズの成果物は `docs/design/01_architecture-evaluation.md` であり、次フェーズ「4. ADR 作成」の唯一の入力となる。
