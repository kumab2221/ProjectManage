# 6. ソフトウェア設計フェーズ（UI / Domain / Infrastructure / Interface / Module）

凍結済みの `docs/requirements/02_system-requirements.md`、`docs/design/00_system-design.md`、`docs/design/01_architecture-evaluation.md` と Accepted な ADR 群を前提に、**実装へ迷いなく橋渡しできるソフトウェア設計書** を作成するフェーズ。設計書は要件 → システム設計 → アーキテクチャ → ADR → ソフトウェア設計 → 実装 の一貫性を担保し、ADR に記録した判断の具体化として扱う。
...
## 目標
要件書、システム設計、アーキテクチャ評価、ADR を確認し、どの設計書を生成するかを決定する。大規模機能であれば複数の設計書を並行して準備する。
