...
%% 既存プロジェクト
CHK -->|No（既存）| ADR_OLD[7.既存ADRの確認]
ADR_OLD --> RQ_EXT[8.新しい要件定義 </br>（既存ADRを参照）]
RQ_EXT --> AR_EXT[9.新しい設計判断</br>（整合性確認）]
AR_EXT --> ADR_EXT[10.ADR作成・更新</br>（必要に応じて）]
ADR_EXT --> DES
IMP --> END[終了]
```
